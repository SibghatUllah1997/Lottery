...
contract Lottery {
  ...
  function buyTicket(uint ticket_price) public payable {
      msg.value == ticket_price;
      require (aTickets != 0 && msg.value == ticket_price && endDate > block.timestamp, "Ticket Price should equal to the Sender value or Available Ticket should not equal to zero.");
    aTickets = aTickets - 1;
  ...
  function getWinner() internal {
    ...
    msg.sender.transfer(prize * 15/100);
    winnerAddr.transfer(prize * 85/100);
  }
}